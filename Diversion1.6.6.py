...
#处理路径中如果带有.和空格的情况
path_to_stream1 = path_to_stream.replace(' ','_')
path_to_streams = path_to_stream1.replace('.','_')
#os.rename(path_to_stream,path_to_streams)
#print(path_to_streams)
#print(path_to_stream)
                ...
                with open(path_to_ffmpeg + "\\" + "Stream_Raw_Infolog.txt",'r',encoding='ISO-8859-1') as f:
                    for line in f:
                        ...
                        if len(video_add_list1):
                            ...
                            pass
                    #for line in f:
                        #A = re.findall('Video:.+\S+', line)
                        #Judge_Video.append(str(re.findall('Video:.+\S+', line)))
                        #print(A)
                    if len(Judge_Video):
                        pass
                    else:
                ...
                with open(path_to_ffmpeg + "\\" + 'Stream_Raw_Infolog.txt', 'r', encoding='ISO-8859-1') as f:
                    for line in f:
                        ...
                        if len(audio_add_list1):
                            ...
                            pass
                    #for line in f:
                    #    B = re.findall('Audio:.+\S+', line)
                     #   Judge_Audio.append(str(re.findall('Audio:.+\S+', line)))
                      #  print(B)
                    if len(Judge_Audio):
                        pass
                    else:
